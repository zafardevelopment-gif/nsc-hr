...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const isSuperAdmin = !user || user.role_type === 'super_admin' || !user.role_type;
        const isAdminOrAbove = isSuperAdmin || user.role_type === 'admin';
        ...
        const [workData, leaveData] = await Promise.all([workRes.json(), leaveRes.json()]);

        setStats(s => ({ ...s, pending: workData.count || 0, pendingLeaves: leaveData.count || 0 }));
        ...
        if (isAdminOrAbove) {
          ...
          const [empData, payData] = await Promise.all([safeJson(empRes), safeJson(payRes)]);
          ...
          setStats(s => ({
            ...
            permanent: perm,
            partTime: pt,
            salaryTotal: payroll.reduce((acc: number, p: { net_pay: number }) => acc + (p.net_pay || 0), 0),
            paid: payroll.filter((p: { status: string }) => p.status === 'paid').length,
          }));
        }

        if (isSuperAdmin) {
          const [chartRes, finEarnRes, finExpRes, finTodayEarnRes, finTodayExpRes, docsRes] = await Promise.all([
          ...
          const [chartJson, finEarnData, finExpData, finTodayEarnData, finTodayExpData, docsData] = await Promise.all([
            safeJson(chartRes), safeJson(finEarnRes), safeJson(finExpRes),
            safeJson(finTodayEarnRes), safeJson(finTodayExpRes), safeJson(docsRes),
          ]);
  ...
  const isSuperAdmin = user.role_type === 'super_admin' || !user.role_type;
  const isAdminOrAbove = isSuperAdmin || user.role_type === 'admin';
  ...
  return (
    <>
      ...
      <div className="page-content">
        ...
        {/* Stats row — all roles */}
        <div style={{ display: 'grid', gridTemplateColumns: isAdminOrAbove ? '1fr 1fr 1fr' : '1fr 1fr', gap: 16 }}>
          <StatCard icon="⚠️" iconBg="#FEF2F2" label="Pending Entries"   value={stats.pending}       trend="review needed" trendDir="down" />
          <StatCard icon="🗓️" iconBg="#EEF2FF" label="Pending Leaves"    value={stats.pendingLeaves} />
          {isAdminOrAbove && <StatCard icon="💰" iconBg="#ECFDF5" label="Salary This Month" value={formatCurrency(stats.salaryTotal)} trend="processed" trendDir="up" />}
        </div>

        {/* Finance + ID widgets — super_admin only */}
        {isSuperAdmin && (
